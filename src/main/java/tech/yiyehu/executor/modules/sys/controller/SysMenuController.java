...
import tech.yiyehu.executor.modules.sys.service.SysMenuService;
...
public class SysMenuController extends AbstractController {
	 ...
	 */
	@GetMapping("/nav")
	public ResultUtil nav(){
		List<SysMenuEntity> menuList = sysMenuService.getUserMenuList(getUserId());
		Set<String> permissions = shiroService.getUserPermissions(getUserId());
		return ResultUtil.ok().put("menuList", menuList).put("permissions", permissions);
	}
	...
	public ResultUtil save(@RequestBody SysMenuEntity menu){
		//数据校验
		verifyForm(menu);
	...
	public ResultUtil update(@RequestBody SysMenuEntity menu){
		//数据校验
		verifyForm(menu);
	...
	public ResultUtil delete(@PathVariable("menuId") long menuId){
		if(menuId <= 31){
		...
		if(menuList.size() > 0){
			return ResultUtil.error("请先删除子菜单或按钮");
		}
